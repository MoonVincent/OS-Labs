...
void setup_vm_final(void) {
    ...
    create_mapping(swapper_pg_dir,(uint64)&_stext,(uint64)(&_stext)-PA2VA_OFFSET,3,11);
    ...
    create_mapping(swapper_pg_dir,(uint64)&_sdata,(uint64)(&_sdata)-PA2VA_OFFSET,32764,7);
    ...
    */
    ...
    for(int i=0;i<sz;i++){
        ...
        if(pgtbl[vpn_2] & 0x1 ){
            ...
            level2 = alloc_page();  //新申请一个页用于存储页表
            memset(level2,0,PGSIZE);
            pgtbl[vpn_2] = ((uint64)level2-PA2VA_OFFSET)>> 2; //地址中的ppn位于12-55位，pte中的ppn位于10-53位
            pgtbl[vpn_2] |= 0x1;
        }
        //设置第三级页表
        ...
        if(level2[vpn_1] & 0x1 ){
            uint64 ppn = (level2[vpn_1] >> 10) & 0xfffffffffff;   //左移10位之后取44位 ppn2：26位 ppn1：9位 ppn0：9位
            level3 = (uint64)(ppn << 12);
            level3 = (uint64)level3 + PA2VA_OFFSET;
        }else{
            level3 = alloc_page();
            level2[vpn_1] = ((uint64)level3-PA2VA_OFFSET) >> 2;
            level2[vpn_1] |= 0x1;
        }
...
int IsMapped(pagetable_t ptb,uint64 addr){
    ...
    uint64 vpn_2 = (addr >> 30) & 0x1ff;     //va中的30-39位为vpn_2
    uint64 *level2;
        

    if(ptb[vpn_2] & 0x1 ){
        uint64 ppn = (ptb[vpn_2] >> 10) & 0xfffffffffff;   //左移12位之后取44位 ppn2：26位 ppn1：9位 ppn0：9位
        level2 = (uint64)(ppn << 12);
        level2 = (uint64)level2 + PA2VA_OFFSET;
    }else{
        return 0;
